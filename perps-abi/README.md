...
Publishing happens from CI (see `.github/workflows/publish-perps-abi.yml`). A `dev` publish updates `environments.testnet` and is tagged `dev` (minor bump). A `main` publish updates `environments.mainnet` and is tagged `latest`; the first mainnet cut is `1.0.0`.
