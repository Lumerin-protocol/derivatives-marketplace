...
contract PerpsSimpleDLL is Initializable, UUPSUpgradeable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    mapping(uint256 => DLL.List) private priceOrdersLongQueue; // FIFO queue of long orders by price
    mapping(uint256 => DLL.List) private priceOrdersShortQueue; // FIFO queue of short orders by price
    ...
    DLL.List private activeBidPrices; // Sorted bid prices (highest first)
    DLL.List private activeAskPrices; // Sorted ask prices (lowest first)
    ...
    function createOrder(uint256 _price, int256 _quantity) external {
        ...
        if (remainingQuantity != 0) {
            ...
            DLL.List storage orderQueue = _priceOrderIds(_price, isBuy);
        ...
        returns (int256 remainingQuantity)
    {
        ...
        DLL.List storage oppositePrices = _isBuy ? activeAskPrices : activeBidPrices;

        if (oppositePrices.sizeOf() == 0) return remainingQuantity;

        uint256 currentPrice = oppositePrices.getNext(0);

        while (currentPrice != 0 && remainingQuantity != 0) {
            if (_isBuy && currentPrice > _limitPrice) break;
            if (!_isBuy && currentPrice < _limitPrice) break;

            uint256 nextPrice = oppositePrices.getNext(currentPrice);
    ...
    function _matchOrdersAtPrice(address _taker, uint256 _price, int256 _remainingQty, bool _isBuy)
        ...
        returns (int256)
    {
        DLL.List storage makerOrderQueue = _priceOrderIds(_price, !_isBuy);

        uint256 orderIdUint = makerOrderQueue.getNext(0);
        while (_remainingQty != 0 && orderIdUint != 0) {
            ...
            orderIdUint = makerOrderQueue.getNext(0);
        }
    ...
    function _removeOrder(bytes32 _orderId, address _participant, uint256 _price, bool _isBid) private {
        _priceOrderIds(_price, _isBid).removeFast(uint256(_orderId));
    ...
    function _priceOrderIds(uint256 _price, bool _isBuy) private view returns (DLL.List storage) {
        if (_isBuy) {
    ...
    function _addPriceLevel(uint256 _price, bool _isBid) private {
        DLL.List storage priceList = _isBid ? activeBidPrices : activeAskPrices;
        ...
        uint256 current = priceList.getNext(0); // Get head
        uint256 prev = 0;
    ...
    function _removePriceLevelIfEmpty(uint256 _price, bool _isBid) private {
        DLL.List storage orderQueue = _isBid ? priceOrdersLongQueue[_price] : priceOrdersShortQueue[_price];
        ...
        returns (int256 filledQuantity, uint256 averageFillPrice, int256 remainingQuantity)
    {
        ...
        DLL.List storage oppositePrices = isBuy ? activeAskPrices : activeBidPrices;
        uint256 currentPrice = oppositePrices.getNext(0);
        ...
        while (currentPrice != 0 && remaining != 0) {
            ...
            while (orderIdUint != 0 && remaining != 0) {
                ...
                orderIdUint = orderQueue.getNext(orderIdUint);
            }

            currentPrice = oppositePrices.getNext(currentPrice);
        }
        ...
        returns (uint256[] memory bidPrices, uint256[] memory askPrices)
    {
        ...
        // Get bid prices
        uint256 current = activeBidPrices.getNext(0);
        for (uint256 i = 0; i < bidCount && current != 0; i++) {
            bidPrices[i] = current;
            current = activeBidPrices.getNext(current);
        }

        // Get ask prices
        current = activeAskPrices.getNext(0);
        for (uint256 i = 0; i < askCount && current != 0; i++) {
            askPrices[i] = current;
            current = activeAskPrices.getNext(current);
        }
