...
| Order margin         | Value reserved for your resting (unmatched) orders             |
| Free / excess margin | Withdrawable collateral above all requirements                 |

Useful views:

- `balanceOf(user)` — total vault balance.
- `getInitialMargin(user)` / `getMaintenanceMargin(user)` — current IM / MM.
- `getOrderMargin(user)` — collateral reserved by resting orders.
