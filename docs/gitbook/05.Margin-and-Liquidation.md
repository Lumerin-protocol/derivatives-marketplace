...
There is no on-chain multi-user batch entry point. To liquidate many
accounts (or clear several orders before a position close) keepers compose
nested `multicallStopOnFailure` bundles — each per-user
clear-and-close is its own inner multicall, so a per-user revert
(`NotLiquidatable`, `OrdersStillOpen`, …) is skipped rather than aborting
the whole batch.
