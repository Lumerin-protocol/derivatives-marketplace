...
describe("positionSessionId", () => {
  test("pads block number to 12 digits and log index to 6 digits", () => {
    const id = positionSessionId(BigInt.fromI32(1), 1);
    assert.assertTrue(id == "000000000001000001");
  });

  test("handles zero block and zero log index", () => {
    const id = positionSessionId(BigInt.zero(), 0);
    assert.assertTrue(id == "000000000000000000");
  });

  test("handles large values", () => {
    const blockNumber = BigInt.fromI32(1234567890);
    const id = positionSessionId(blockNumber, 999999);
    // block padded to 12 digits: 001234567890, logIndex to 6: 999999
    assert.assertTrue(id == "001234567890999999");
  });

  test("does not truncate when block exceeds 12 digits", () => {
    const blockNumber = BigInt.fromString("9999999999999");
    const id = positionSessionId(blockNumber, 0);
    assert.assertTrue(id == "9999999999999000000");
  });
});
